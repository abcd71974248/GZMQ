package com.hotsun.mqxxgl.gis.sync;
...
public class GDBUtil {
  ...
  private static String DEFAULT_FEATURE_SERVICE_URL="";
  ...
  private static final int[] FEATURE_SERVICE_LAYER_IDS = { 0, 1, 2 };

  protected static final String TAG = "GDBUtil";

  private static GeodatabaseSyncTask gdbTask;

  private static String DEFAULT_GDB_PATH = "";

  private static String gdbFileName = "";

  public GDBUtil(final GisBaseActivity activity){
    DEFAULT_FEATURE_SERVICE_URL = activity.getResources().getString(R.string.fe_server);
    DEFAULT_GDB_PATH =  ResourcesUtil.getInstance().getFolderPath(activity,ResourcesUtil.geoPath);
    gdbFileName = DEFAULT_GDB_PATH + ResourcesUtil.geoFile;
  }

  /**
   * Go back online
   */
  public static void goOnline(final GisBaseActivity activity, final MapView mapView) {
            ...
            case DialogInterface.BUTTON_POSITIVE:
				...
				File[] fileDelete = dir.listFiles(new FileFilter() {
					...
					public boolean accept(File pathname) {
						return (pathname.getName().startsWith("offlinedata"));
					}
  ...
  private static void finishGoingOnline(final GisBaseActivity activity, final MapView mapView){
   ...
   */
  public void downloadData(final GisBaseActivity activity) {
  ...
  private static void downloadGeodatabase(final GisBaseActivity activity, final MapView mapView) {
    ...
    // load the geodatabase from the device
      ...
      GisBaseActivity.progress.dismiss();
      showProgress(activity, false);
    }
  ...
  private static void requestGdbFromServer(GeodatabaseSyncTask geodatabaseSyncTask,
      final GisBaseActivity activity, final MapView mapView, FeatureServiceInfo fsInfo) {
  ...
  private static void addLocalLayers(final MapView mapView, String gdbPath, final GisBaseActivity activity) {
  ...
  public void synchronize(final GisBaseActivity activity) {
    showProgress(activity, true);
   ...
   * @throws Exception
   */
  private static void doSyncAllInOne(final GisBaseActivity activity) {

    try {
      // create local geodatabase
      if(!isGeoDatabaseLocal()){
        showMessage(activity, "本地同步文件不存在");
        return;
      }
   ...
   */
  private static boolean isBasemapLocal() {
   ...
   */
  private static boolean isGeoDatabaseLocal() {
    File file = new File(gdbFileName);
    return file.exists();
  }

  static void showProgress(final GisBaseActivity activity, final boolean b) {
  ...
  static void showMessage(final GisBaseActivity activity, final String message) {
    activity.runOnUiThread(new Runnable() {
